...
void Ntupler::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup){
	...
	if(GoodRun){
		...
		if (HasTrigger || HasTrigger2 || HasTriggerBase2){
			...
			nGoodCA8PFJets=0;
		 	nCleanCA8PFJets=0;
			nGoodCA8PrunedPFJets=0;
		 	nCleanCA8PrunedPFJets=0;
			nCA8PrunedPFJets=0;
			nCA8PFJets=0;
			nPFJets=0;
			...
			massTriplet.clear();
			nTriplets=0; 
			q=0; //basically just triplet counting
			IsVtxGood = 0; 
...
void Ntupler::beginJob() {
	...
	h_nGoodVtx = new TH1F("nVtx","Number of Vertices",51,-0.5,50.5);
	...
	for(int i=0; i<6; i++){
		...
		v_PFjet_m.push_back(new TH1F(hNAME,hTITLE,100,0,500));
...
void Ntupler::beginRun(edm::Run const&, edm::EventSetup const&)
{
}

// ------------ method called when ending the processing of a run  ------------
void Ntupler::endRun(edm::Run const&, edm::EventSetup const&)
{
}
...
void Ntupler::endLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&)
{
}
...
void Ntupler::UseJson( vector<int > GoodRuns, vector<int > GoodLumiStart,  vector<int > GoodLumiEnd,  Int_t nGoodRuns , int run, int lumis){

	if (_isData) {
		GoodRun=kFALSE;
		for (int ii=0;ii<nGoodRuns;++ii){
			if (run == GoodRuns[ii]){
				if (lumis >= GoodLumiStart[ii] && lumis <=GoodLumiEnd[ii]) GoodRun = kTRUE;
			}
		}
	} else {  
		GoodRun = kTRUE;
...
void Ntupler::getTriggerDecision(const edm::Event& iEvent, std::map<std::string, bool>& TriggerMap) {
	...
	for (std::map<std::string, bool>::iterator It = TriggerMap.begin(); It != TriggerMap.end(); ++It) cout << It->first << ", ";
	cout << " with bad index " << triggerIndex << endl;
}


void Ntupler::GetSUSYpoint(const edm::Event& iEvent, const edm::EventSetup& iSetup){
...
vector<string> Ntupler::split(string fstring, string splitter){
	...
	if(fstring.find(splitter) == string::npos){
		...
		while(afterSplitter.find(splitter) != string::npos){
			...
			if(afterSplitter.find(splitter) == string::npos) returnVector.push_back(afterSplitter);
		}
		return returnVector;
	}
}
...
void Ntupler::DoJetID(const edm::Event& iEvent,const edm::EventSetup& iSetup, std::string PatJetType){
	...
	for (unsigned int j=0; j<PatJets->size(); j++) {
		...
		if ((*PatJets)[j].pt() > 20.0  && fabs((*PatJets)[j].eta()) < 3.0){
			
			if (_debug) std::cout << "Jet #" << j << " Passes pT cut of " << _jetptcut << " and eTa cut of " << _etacut << " with pT: " <<  (*PatJets)[j].pt() << ", and eTa: " << (*PatJets)[j].eta() << std::endl;
			...
			if(jetID){
				if (_debug) std::cout<<"After JetID" << "Jet #" << j << " Passes pT cut of " << _jetptcut << " and eTa cut of " << _etacut << " with pT: " <<  (*PatJets)[j].pt() << ", and eTa: " << (*PatJets)[j].eta() << std::endl;
...
void Ntupler::GetTruePileUp(const edm::Event& iEvent){
...
void Ntupler::DoVertexID(const edm::Event& iEvent){
	...
	for (size_t i=0; i<recVtxs->size(); ++i) 
		if (!((*recVtxs)[i].isFake())) {
			if ( ((*recVtxs)[i].ndof() > 4) && (fabs( (*recVtxs)[i].z()) <= 24) && ((*recVtxs)[i].position().rho() <= 2) ){
				CountVtx++;
...
void Ntupler::DoElectronID(const edm::Event& iEvent){
	...
	for (std::vector<pat::Electron>::const_iterator Electron = PatElectrons->begin(); Electron != PatElectrons->end(); ++Electron) {
		...
		if(Electron->userInt("electronUserData:seedSeverityLevel")==3 || Electron->userInt("electronUserData:seedSeverityLevel")==4 || Electron->userInt("electronUserData:seedSeverityLevel")==5) continue;
...
void Ntupler::DoMuonID(const edm::Event& iEvent){
...
void Ntupler::GetPFIso(vector<Muon >fCleanMuons){

	for (size_t im = 0; im != fCleanMuons.size(); ++im) {

		// Get the PFGamma isolationi
...
void Ntupler::DoMETID(const edm::Event& iEvent){
...
void Ntupler::DoCleanUp(vector<Muon >fGoodMuons,vector<Electron >fGoodElectrons,vector<Photon >fGoodPhotons,vector<Jet >fGoodJets){
...
void Ntupler::GetMCTruth(const edm::Event& iEvent){
