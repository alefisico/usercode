...
####################################################
###
###  Simple tcsh script to Ntuplizer the samples
...
###
###  How to run: 
###  source run_pattuplozer_job.sh 100 250
...
set Main_Dir = /uscms/home/algomez/work/CMSSW_5_3_3/src/TopQuarkAnalysis/TopPairBSM/test/Stops/		# Main Dir
set Name = st2_h_bb_st1_${Process}_${stop2}_${stop1}						# Name of the process
set Working_Dir = ${Main_Dir}/${Name}								# Working directory
set OutputFiles_Dir = /eos/uscms/store/user/algomez/Stops/PATTuples/${Name}/			# Ouput directory
set InputFiles_Dir = ${Name}_AOD/								# Name of the folder of the input files
set Input_Dir = /pnfs/cms/WAX/11/store/user/algomez/$InputFiles_Dir  				# Complete name of the dir of the input files
...
##############################################
/bin/rm -f ${Name}_cfg.py
cat > ${Name}_cfg.py << EOF
...
EOF
########################################################
######### Search for the names of the input files
########################################################
foreach ifile (`ls -1 $Input_Dir`)
set files = ( \'/store/user/algomez/$InputFiles_Dir/$ifile\', )
echo $files >> ${Name}_cfg.py
end

######### Continues writing the python config file
cat >> ${Name}_cfg.py << EOF
    ]
...
if options.useData :
    ...
    if options.writeFat :
        ...
        process.out.fileName = cms.untracked.string('$OutputFiles_Dir/${Name}_'+options.tlbsmTag + '_mc_1.root')
...
open('${Name}_junk_1.py','w').write(process.dumpPython())
EOF

########################################################
######### I decided to split the jobs in 4.
######### Here, I am creating the condor script.
########################################################
foreach version(1 2 3 4 )
...
########################################################
/bin/rm -f ${Name}_${version}.sh 
cat > ${Name}_${version}.sh << EOF
#!/bin/tcsh -f
