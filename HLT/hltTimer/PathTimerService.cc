...
namespace edm {
	...
	// Checks Time System
        static double getTime()  {		// gets Time System (static)
            struct timeval t;			// introduces a type timeval t
            if(gettimeofday(&t,0)<0)		// checks if system time is greater than 0
                throw cms::Exception("SysCallFailed","Failed call to gettimeofday");
      
            return (double)t.tv_sec + (double(t.tv_usec) * 1E-6);	// returns time in seconds plus microseconds
        }

	// Initialize PathTimerService with CPU Time
        edm::CPUTimer* PathTimerService::_CPUtimer = 0;

	// PathTimerService Constructor
        PathTimerService::PathTimerService(const ParameterSet& iPS, ActivityRegistry&iRegistry):	// calls two parameters and references them 
            total_event_count_(0),		// initializes total_event_count
            _perfInfo(new HLTPerformanceInfo)	// stores raw memory to HLTPerformanceInfo
	// Calls class PathTimerServices members
        {
            iRegistry.watchPostBeginJob(this,&PathTimerService::postBeginJob);	// this: reference to  
            ...
            _myPS=iPS;				// change ParameterSet reference name

            if (!_CPUtimer) _CPUtimer = new edm::CPUTimer();	//if CPUTimer doesn't start calls it again
        }

	// PathTimerService Destructor
        ...
        void PathTimerService::postBeginJob() {
            ...
            edm::Service<edm::service::TriggerNamesService> tns;		// calls Trigger names
