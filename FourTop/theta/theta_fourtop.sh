...
foreach Process (400 500 600 700 800 900 1000)
#foreach Process (800 900 1000)
set version = v1
#set version = Likelihoodv3
#set results = results108
#set results = results108/CSVM1/
...
#set Sample_Dir = ${Main_Dir}/${results}${version}_v0/ #_cls/
#set Sample_Dir = ${Main_Dir}/results108_CSVM1_${version}/
set Sample_Dir = ${Main_Dir}/results110_Nbjets3_Likelihood${version}/ #${version}/
...
/bin/rm -f analysis_${version}_tttt_Gh${Process}.py
cat > analysis_${version}_tttt_Gh${Process}.py << EOF
...
def get_model():
    ...
    #model = build_model_from_rootfile('/uscms/home/algomez/nobackup/files/fourtop/resultsTreeAnalyzer/${results}/BDTResponse${version}/mu_disc${Process}_templates.root', include_mc_uncertainties = True)
    #model = build_model_from_rootfile('/uscms/home/algomez/nobackup/files/fourtop/resultsTreeAnalyzer/${results}/MLPResponse${version}/mu_disc${Process}_templates.root', include_mc_uncertainties = True)
    model = build_model_from_rootfile('/uscms/home/algomez/nobackup/files/fourtop/resultsTreeAnalyzer/${results}/LikelihoodResponse${version}/mu_disc${Process}_templates.root', include_mc_uncertainties = True)
    #model = build_model_from_rootfile('/uscms/home/algomez/nobackup/files/fourtop/resultsTreeAnalyzer/${results}/mu_stjet_templates.root', include_mc_uncertainties = True)
...
#plot_exp.write_txt('bayesian_limits_expected_MLP'+points[0][0]+'_${version}.txt')
#plot_obs.write_txt('bayesian_limits_observed_MLP'+points[0][0]+'_${version}.txt')
plot_exp.write_txt('bayesian_limits_expected_Likelihood'+points[0][0]+'_${version}.txt')
plot_obs.write_txt('bayesian_limits_observed_Likelihood'+points[0][0]+'_${version}.txt')
...
/bin/rm -f tttt_Gh${Process}_limits_${version}.sh
cat > tttt_Gh${Process}_limits_${version}.sh << EOF
#!/bin/tcsh -f
...
../../../theta-auto.py analysis_${version}_tttt_Gh${Process}.py 
EOF

#chmod +x tttt_Gh${Process}_limits_${version}.sh

/bin/rm -f tttt_Gh${Process}_limits_${version}.condor
cat > tttt_Gh${Process}_limits_${version}.condor << EOF
universe = vanilla
Executable = $Sample_Dir/tttt_Gh${Process}_limits_${version}.sh 
Requirements = Memory >= 199 &&OpSys == "LINUX"&& (Arch != "DUMMY" )&& Disk > 1000000
Should_Transfer_Files = YES
WhenToTransferOutput = ON_EXIT
Output = $Sample_Dir/tttt_Gh${Process}_limits_${version}.stdout
Error = $Sample_Dir/tttt_Gh${Process}_limits_${version}.stderr
Log = $Sample_Dir/tttt_Gh${Process}_limits_${version}.condorlog
...
condor_submit tttt_Gh${Process}_limits_${version}.condor
