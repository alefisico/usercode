   ...
   for (int imva=0; imva<Nmvas; imva++) {
      if (iuse[imva]) {
         ...
         if (bulkname[imva] == "BDT"          ) {
            ...
            hist[imva] = new TH1D( "MVA_BDT",           "MVA_BDT",           nbin, -1.0, 1.0 );
         }
...
void ClassApplication_Version2(string topsettingfile){
   ...
   {   
      ...
      string tmpbackttbarScaledown = "results_ttbarScaledown.root";
      string tmpbackttbarScaleup = "results_ttbarScaleup.root";
      ...
      tmpback.push_back(tmpbackbtagup);
      tmpback.push_back(tmpbackbtagdown);
      tmpback.push_back(tmpbackdata);
      tmpback.push_back(tmpbackttbarMatchdown);
      tmpback.push_back(tmpbackttbarMatchup);
      tmpback.push_back(tmpbackttbarScaledown);
      tmpback.push_back(tmpbackttbarScaleup);
