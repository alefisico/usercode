...
Description: Moves a bunch of files from a directory to my elog (at cern) and generate html img for every file.
		Requieres a file .moveFiles.config in the home area.
		Original code here: http://home.fnal.gov/~cplager/log/elogHowTo/howTo.html#movingfiles
'''
...
filesMap = {}
################  My modifications
tar = tarfile.open(dateKey+"files.tar.gz", "w:gz")
################
for sourceFile in files:
    ...
    ######## add files into tar file
    copyFile = "cp %s %s " % ( sourceFile, targetFile )
    os.system(copyFile)
    tar.add( targetFile )
...
if onHost:
	...
	tarSourceFile = dateKey+"files.tar.gz"
        cmd = "scp %s %s %s:%s" % (user, tarSourceFile, \
                                   hostComputer, targetDir)
	print cmd
        os.system (cmd)
	rmtmp = "rm %s" % ( dateKey+"*" )
	os.system( rmtmp )
